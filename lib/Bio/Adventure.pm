...
has adventure => (is => 'rw', default => undef);
...
## Test to see if I can move defaults to the bottom
...
=head1 SYNOPSIS
    ...
    Bio::Adventure.pm  Methods to simplify creating job scripts and submitting
 them to a
...
$ENV{PATH}.") unless($check);
  ...
  #DEF: foreach my $k (keys %{$default_vars}) {
  #      for my $ignore (@ignored) {
  #          next DEF if ($k eq $ignore);
  #      }
  #      $defaults{$k} = $default_vars->{$k};
  #      $class->{$k} = $default_vars->{$k};
  #  }
    if (defined($class->{slots_ignored})) {
        @ignored = split(/[,]/, $class->{slots_ignored});
    }
  IGN: foreach my $k (keys %{$class}) {
        for my $ignore (@ignored) {
            next IGN if ($k eq $ignore);
        }
        $defaults{$k} = $class->{$k};
    }
    ...
    if ($#ARGV > 0) {
        ...
        make_path($class->{output_base}, {verbose => 0}) unless (-d $class->{output_base});
        my $out = FileHandle->new(qq">>$class->{output_base}/log.txt");
        my $d = qx'date';
        chomp $d;
...
sub Read_Genome_Fasta {
    ...
    if (defined($options->{genome})) {
        $input_file = $options->{genome};
    } else {
        if (defined($options->{input})) {
            $input_file = $options->{input};
        } elsif (defined($options->{fasta})) {
            $input_file = $options->{fasta};
        }
    }
    my $fasta_name = basename($input_file, ('.gz', '.xz', '.bz2'));
    $fasta_name = basename($fasta_name, ('.fasta', '.fa', '.ffn', '.fsa',));
    my $genome_file = qq"$options->{basedir}/$options->{script_dir}/pdata/${fasta_name}.pdata";
    ##if (-r $genome_file) {
    ##    $chromosomes = lock_retrieve($genome_file);
    ##} else {
    my $fh = Bio::Adventure::Get_FH(input => $input_file);
    my $input_genome = Bio::SeqIO->new(-fh => $fh, -format => 'Fasta');
...
=head2 C<Record_Options>

 After submitting a job to a local shell/cluster/etc, write down some of the options used.

 My thinking is to be able to, in future invocations, skip passing some options like input.
...
";
    ...
    my $option_file = '';
    if ($options->{language} eq 'perl') {
        my $pd_dir = qq"$options->{basedir}/$options->{script_dir}/pdata";
        if (!-d $pd_dir) {
            my $created = make_path($pd_dir);
        }
        ...
        if ($options->{pdata}) {
            ...
            $option_file = File::Temp->new(
                ...
                DIR => qq"$options->{basedir}/$options->{script_dir}/pdata",
                UNLINK => 0,
                SUFFIX => '.pdata',);
